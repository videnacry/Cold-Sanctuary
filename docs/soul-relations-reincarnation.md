...
**Dinámica (PR #84) — `MoodDynamics`:** los humores no se quedan en la base: el **estrés (cortisol) es químico y SUBE con el estado** — fatiga (mucho trabajo/de pie), sueño, hambre (`Metabolism.Appetite`) y reservas bajas (glucosa/minerales) — y BAJA descansado/saciado; la `serotonina` cede ante el cortisol; `Anima.stress` lo refleja. Así los **estados de ánimo emergen del estado**: el mal humor de Goluis es **situacional** (`Goluis.UnderPressure` = override manual O `fatigue`/`stress` altos), no un rasgo. La `sensibilidad` gobierna la reactividad. **Desgaste por trabajo (PR #85) — `Exertion`/`ExertionCost` (vía A):** cada acción de trabajo declara su coste (`glucose`/`minerals`/`fatigue`/`sleepiness`); `Exertion.Apply(Anima)` lo descuenta de `Humores` (glucosa/calcio), sube `sleepiness` y `MoodState.fatigue`. Enganchado en `VirtualTask` (`worker` + `exertionPerStep`) → `ProductionOrder` (con `stepExertion[]` paralelo: cargar/forjar pesan más que abrir una puerta) y `StockingTask` (coger/colocar). Así «estar de pie o hacer fuerza» **gasta reservas** que `MoodDynamics` convierte en cortisol → el obrero se estresa solo. Sandbox `Desgaste_AUTO`. **El jugador ya ES un `Anima`** (migrado 2026-07-28): **integrado (PR #86)** — `MoodDynamics` corre sobre cualquier `Anima` (con `Mind` usa humores; **sin `Mind`, como el jugador**, deriva el estrés de la fatiga/sueño/hambre universales), y `Exertion` gasta la **fatiga universal** (`Anima.mentalFatigue`). En el builder el jugador lleva `MoodDynamics` y es el `worker` de la receta de cocina → **cocinar cansa y estresa al propio jugador**.

**Pensamientos base de especie (PR #89):** el arquetipo de MENTE declara `basePhrases` (innatas por especie, `PhraseCategory.Especie`, tono acorde). Se siembran en `Mind.thoughts` al componerse (`SoulComposition` por la mente dominante; `Animal.ApplySpeciesArchetype` por su especie) → un oso "piensa como oso" de arranque. Data-driven en `Archetypes`/`PhrasePools`, sin clases `WolfMind`/`BearMind`. `Archetypes.BaseThoughtsOf(especie)` / `Mind.SeedThoughts`.
