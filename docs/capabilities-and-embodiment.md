...
| **D3b** (en curso) | **La selección CONDUCE la acción** | **[`volition-selection-engine.md`](volition-selection-engine.md)**. **D3b1 ✔ (#136):** `DesireCatalog`+`Volition`+flag `AiBrain.useVolition` (OFF, paridad: `eat`+reflejo de amenaza). Falta D3b2 (más deseos + amenaza como deseo; flip del flag) y D3b3 (hechizos como acciones → activan D3a/E2) | `Mind`/`Animal`/`AiBrain` |
