...
- **28-ago (PR #150):** **primer sandbox `*_AUTO` con `TestProbe` — `PheromoneFieldTest`** (testing-checklist §26/§29). Self-test AUTOEJECUTABLE de la rejilla en pura lógica (sin NavMesh/Animals): se auto-crea un `PheromoneField` y asevera por `TestProbe` depósito/lectura/lejos-es-0/gradiente-hacia-el-depósito/decaimiento-temporal/`volumetric`-separa-por-`y`, luego se destruye. `SampleSceneBuilder` lo añade como `NavTests_AUTO` → corre al entrar en Play y el compañero lee el veredicto en `Editor.log` (grep `[TEST] ... PheromoneField`, `SUMMARY: X PASS / Y FAIL`). **Enciende** el primer test interactivo que el smoke-test no tocaba (la rejilla era invisible). Los checks de CONDUCTA (animal deriva al rastro / huye) necesitan NavMesh horneado → sandbox/misión aparte (siguiente).
