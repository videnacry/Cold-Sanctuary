...
5. **Disolver `CompanionBase`** — 🟡 **PREPARADO (PR #74)**: `BondPillar` (implementa `IBondable` por componente, como `WorldBondable`) extrae el vínculo/proximidad → un compañero puede ser `Anima + SoulComposition + BondPillar` sin heredar `CompanionBase`. Demo `Panterilia_SinClase` (arquetipo Panterilia con sus 12 valores reales). *Falta:* migrar Goluis/Panterilia/Gohageneis/Irosene a este patrón (capturar sus `Base*` como arquetipos) y **retirar** `CompanionBase` (con compilador). Reencarnación/transformación = añadir/quitar arquetipos.
