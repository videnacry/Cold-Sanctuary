...
public class TraceFieldTest : MonoBehaviour, ITestUnit
{
    ...
    public IEnumerator Run()
    {
        ...
        // Depósito + lectura.
        TraceField.Leave(a, CH, 10f);
        TestProbe.Greater("deposito legible", TraceField.Sniff(a, CH), 0f);
        TestProbe.Check("lejos = 0", TraceField.Sniff(new Vector3(1000f, 0f, 1000f), CH) == 0f);

        // Gradiente: desde una celda vecina, Trail apunta HACIA el depósito.
        Vector3 near = a + new Vector3(4f, 0f, 0f);
        Vector3 g = TraceField.Trail(near, CH);
        ...
        float before = TraceField.Sniff(a, CH);
        yield return new WaitForSeconds(1.5f);
        float after = TraceField.Sniff(a, CH);
        ...
        TraceField.Leave(lo, CH, 5f);
        TraceField.Leave(hi, CH, 10f);
        TestProbe.Near("3D: la profundidad separa celdas (bajo)", TraceField.Sniff(lo, CH), 5f, 0.5f);
        TestProbe.Near("3D: la profundidad separa celdas (alto)", TraceField.Sniff(hi, CH), 10f, 0.5f);
