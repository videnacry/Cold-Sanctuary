...
- [`docs/cria-simulation.md`](docs/cria-simulation.md) — **el área de CRÍA (corazón del santuario)**: cuidar crías para ganar el bond (se gana, no se da); virtualización (limpiar→abastecer→rutina leer-estado/calmar/alimentar/asear/arrullar→nido) reusando fauna + `CarryToRefuge`/`WeakOne`. Va antes de la Mecánica (domesticación pre-metales).
