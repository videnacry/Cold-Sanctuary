...
| Ropa/crafting + Composición | `Assets/Scripts/Clothing/`, `Assets/Scripts/Composition/` | `ClothingRecipe`/`ClothingCraftingArea` (crafting, aún no entrega item). **`CharacterComposition`+`CompositionPart`** (fase 1): partes slotables (adornos/ropa) → visual + la defensa de la ropa suma a `Anima.armadura` (que `Predation` lee). `ClothingSlot` ampliado (Hair/Eyebrows/Eyes). Fases 2-3: base/efectivo + miembros con stats. Ver `docs/stats-as-truth.md` §5 |
