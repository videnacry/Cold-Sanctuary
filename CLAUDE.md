...
| Magia / metabolismo (reservas) | `Assets/Scripts/Transformation/` (magia) + `Assets/Scripts/Composition/` (metab.) | **Bucle comer→lanzar.** `MagicReserves` (pools por elemento + reserva de **energía** J; `Pay(costs,energy)`), `Grimoire` (hechizos aprendidos; el 1º `Unlock()`ea las pools), `QuarkReserve` (S4: quarks flexibles→cualquier elemento/energía, 1 g≈1,8e24 quarks), `FireSpell` (coste físico real: químico C+H+ignición / masa-energía E=mc²), `MagicAura`. `Metabolism`/`Constitution` (nutriente→elemento→stats; exceso→reservas). **Opt-in, sin sandbox aún.** Ver `docs/magic-metabolism-progression.md` |
